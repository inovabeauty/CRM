...
export async function runErpSync(customCrmClient = null) {
  ...
  for (const ec of (erpClients || [])) {
    ...
    // APLICAÇÃO DO VÍNCULO OU INSERÇÃO
    if (match) {
      // 1. ATUALIZA SALÃO EXISTENTE NO CRM
      const updatePayload = {
      ...
      if (novoStatusFunil && (!match.status_funil || match.status_funil === 'prospect' || match.status_funil === 'negociacao')) {
      ...
      // 2. CRIA NOVO SALÃO NO CRM (CASO DA ALICE BLOND HAIR)
      // Determina coordenadas padrão pelo centroide da cidade + jitter de dispersão
      const cityKey = (ec.cidade || 'caxias').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim();
      const centroid = CITY_CENTROIDS[cityKey] || CITY_CENTROIDS['caxias'];
      
      // Jitter de ~100-300 metros para não sobrepor pinos pendentes no mesmo pixel
      const jitterLat = (Math.random() - 0.5) * 0.006;
      const jitterLng = (Math.random() - 0.5) * 0.006;

      const newClientPayload = {
        nome: ec.nome_estabelecimento || 'Salão ERP Sem Nome',
        responsavel: ec.nome_responsavel || null,
        whatsapp: ec.telefone || null,
        cidade: ec.cidade || 'Caxias',
        endereco: ec.endereco ? `${ec.endereco}, ${ec.cidade}` : `Endereço a confirmar (${ec.cidade || 'Caxias'})`,
        categorias: ['Salão de Beleza'],
        status_funil: novoStatusFunil,
        latitude: centroid.lat + jitterLat,
        longitude: centroid.lng + jitterLng,
        localizacao_pendente: true, // FLAG VISUAL PARA O VENDEDOR FIXAR NA ROTA!
        erp_cliente_id: ec.id,
        ...
        valor_total_comprado: totalGasto,
        qtd_compras: comprasDoCliente.length,
        linha_interesse: linhaDetectada || 'Olenka Cosméticos',
        observacoes: `[Sincronização ERP] Cadastrado via compras do ERP (${comprasDoCliente.length} compras faturadas). Localização física pendente de confirmação na rota.`
      };

      await db.from('clientes').insert(newClientPayload);
      createdCount++;
      matchReports.push({
        status: 'CRIADO_NOVO',
        erp: ec.nome_estabelecimento,
        crm: newClientPayload.nome,
        cidade: ec.cidade,
        motivo: 'Cliente novo do ERP inserido com localização pendente de rota',
        compras: comprasDoCliente.length,
        inadimplente: temBoletoAtrasado
      });
    }
