...
export async function runErpSync(customCrmClient = null) {
  ...
  const [
    ...
    { data: erpPagamentos, error: errPagamentos }
  ] = await Promise.all([
    ...
    erpSupabase.from('boletos').select('id, cliente_id, venda_id, valor, data_vencimento, data_pagamento, status, numero_parcela, total_parcelas, observacao'),
    erpSupabase.from('pagamentos_receber').select('id, boleto_id, valor_recebido, data_pagamento')
  ]);
  ...
  if (errBoletos) {
    console.warn('Aviso ao consultar boletos do ERP:', errBoletos.message);
  }
  ...
  // 4. AGRUPA PAGAMENTOS E BOLETOS POR CLIENTE DO ERP COM CÁLCULO DE SALDO REAL
  const pagamentosPorBoleto = (erpPagamentos || []).reduce((acc, p) => {
    if (p.boleto_id) {
      acc[p.boleto_id] = (acc[p.boleto_id] || 0) + (parseFloat(p.valor_recebido) || 0);
    }
    return acc;
  }, {});

  const infoFinanceiraPorCliente = {};
  let totalBoletosAbertosGeral = 0;
  let totalValorAbertoGeral = 0;

  for (const b of (erpBoletos || [])) {
    if (b.status === 'cancelado' || b.status === 'pago') continue;
    const pago = pagamentosPorBoleto[b.id] || 0;
    const saldo = Math.max(0, parseFloat(b.valor || 0) - pago);
    if (saldo <= 0) continue;

    // Considera vencido se o status for 'vencido' OU se for 'aberto' com vencimento anterior à data de hoje
    const vencido = (b.status === 'vencido') || (b.status === 'aberto' && b.data_vencimento < hojeIso);
    const cId = b.cliente_id;

    if (!infoFinanceiraPorCliente[cId]) {
      infoFinanceiraPorCliente[cId] = {
        temBoletoAtrasado: false,
    ...
    const info = infoFinanceiraPorCliente[cId];
    info.temBoletoAberto = true;
    info.valorTotalAberto += saldo;
    totalBoletosAbertosGeral++;
    totalValorAbertoGeral += saldo;

    if (vencido) {
      info.temBoletoAtrasado = true;
      info.valorInadimplente += saldo;
    }

    info.boletos.push({
      id: b.id,
      parcela: b.numero_parcela ? `${b.numero_parcela}/${b.total_parcelas || 1}` : '1/1',
  ...
  for (const ec of (erpClients || [])) {
    ...
    const infoFin = infoFinanceiraPorCliente[ec.id] || {
      temBoletoAtrasado: false,
      temBoletoAberto: false,
      valorInadimplente: 0,
      valorTotalAberto: 0,
    ...
    const temBoletoAtrasado = infoFin.temBoletoAtrasado;
    ...
    if (match) {
      ...
      const updatePayload = {
        ...
        valor_inadimplente: infoFin.valorInadimplente,
        boletos_pendentes: infoFin.boletos,
  ...
  return {
    ...
    clientesComPendencias: Object.values(infoFinanceiraPorCliente).filter(c => c.temBoletoAberto).length,
    boletosAbertosCount: totalBoletosAbertosGeral,
    totalValorAberto: totalValorAbertoGeral,
