...
export default function Navbar({
  ...
  // Autocomplete buscando no Supabase
  useEffect(() => {
    ...
    const delaySearch = setTimeout(async () => {
      try {
        const { data } = await supabase
          .rpc('buscar_clientes', { search_term: searchQuery })
        ...
        // Fallback para ilike se a rpc falhar
        const { data } = await supabase
          .from('clientes')
          .select('id, nome, latitude, longitude, categorias, whatsapp, status_funil, cidade, boleto_atrasado, data_ultima_compra, linha_interesse, melhor_dia_compra, observacoes, ultimas_compras, responsavel, instagram, telefone_alternativo, data_ultima_interacao, tipo_ultima_interacao, data_retorno, endereco, localizacao_pendente, erp_cliente_id, valor_total_comprado, qtd_compras')
  ...
  return (
    <header className="bg-base-100/95 backdrop-blur-md border-b border-base-200 z-[9999] shrink-0 sticky top-0 shadow-xs transition-colors">
      ...
      <div className="px-3 md:px-4 py-2 flex items-center justify-between gap-2">
        ...
        {/* Ações Rápidas (Rota, Fechamento, Sincronização ERP, Equipe, Alternador de Tema, Usuário) */}
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          >
            ...
            <span className="hidden md:inline">{isSyncingErp ? 'Sync...' : 'Sync ERP'}</span>
          </button>

          {/* Botão Gestão de Equipe (Apenas para Gestor) */}
          {isGestor && (
            <button
              type="button"
          ...
          <button
            ...
            className="btn btn-xs sm:btn-sm btn-ghost hover:bg-amber-500/15 border border-amber-500/30 text-amber-500 font-bold text-[11px] gap-1 shadow-xs rounded-xl"
          ...
          {user && (
            <div className="dropdown dropdown-end">
              <label tabIndex={0} className="btn btn-xs sm:btn-sm btn-ghost hover:bg-base-200 border border-base-300 rounded-xl px-2 gap-1.5 flex items-center cursor-pointer">
                ...
                <span className={`badge badge-xs font-semibold ${isGestor ? 'badge-primary text-white' : 'badge-neutral'}`}>
                  {isGestor ? 'Gestor' : 'Vendedor'}
                </span>
              </label>
              <ul tabIndex={0} className="dropdown-content z-[100000] menu p-2 shadow-2xl bg-base-100 rounded-box w-52 border border-base-300 mt-2 text-xs">
                ...
                <li>
                  <button onClick={onOpenPwaInstall} className="flex items-center gap-2 text-amber-500 font-medium">
                    <span>📲</span> Instalar App Celular
                  </button>
                </li>
                {isGestor && (
                  <li>
      ...
      <div className="px-3 md:px-4 pb-2">
        <div className="relative w-full">
          <div className="relative flex items-center">
            ...
            <input
              type="text"
              placeholder="Buscar salão, dona ou WhatsApp..."
                  ...
                  <div className="font-bold text-sm truncate text-base-content flex items-center justify-between gap-1.5">
                    ...
                    {c.status_funil && (
                      <span className="badge badge-xs badge-ghost text-[10px] shrink-0 font-medium">
                        {c.status_funil === 'cliente_ativo' ? '🟢 Ativo' : c.status_funil === 'negociacao' ? '🟡 Negociação' : c.status_funil === 'alerta_resgate' ? '🔴 Resgate' : '🔵 Prospect'}
                      </span>
                    )}
                  </div>
                  <div className="text-xs text-base-content/65 flex justify-between mt-0.5">
                    <span className="truncate pr-2">
                      {c.cidade} {c.responsavel ? `• Dona: ${c.responsavel}` : ''}
                    </span>
